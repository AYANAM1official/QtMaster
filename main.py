...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("无人超市上位机 V2.5 (模拟扫码选择器)")
    ...
    def init_ui(self):
        ...
        self.btn_scan_test.clicked.connect(self.open_scan_simulation) # 修改这里
    ...
    def open_scan_simulation(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            # 3. 如果用户选了某个商品，获取ID并发送
            target_id = dialog.selected_id
            if target_id:
                # 构造协议: CMD:SCAN,ID:xxxxxxxx
    ...
    def open_product_editor(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if self.pm.save_data(new_data):
                ...
                QMessageBox.information(self, "成功", "商品库已保存，准备同步...")
    ...
    def perform_full_sync(self):
        ...
        self.lbl_status.setText("⏳ 正在同步数据...")
        
        self.worker.send("CMD:SYNC_START")
        all_items = self.pm.get_all_list()
        for item in all_items:
            cmd = f"CMD:SYNC_DATA,ID:{item['id']},PR:{item['price']},NM:{item['name']}"
            self.worker.send(cmd)
            
        self.worker.send("CMD:SYNC_END")
        self.append_log(f"同步完成，共发送 {len(all_items)} 条数据")
